...
license: proprietary
version: "1.0.0"
tags: ["saas", "funnel", "asset-generation", "orchestrator", "funnel-strategy", "multi-asset"]
...
- `browser-verifier` (S4-Testing) — every generated HTML page in the funnel package should be rendered and checked before handoff to the client
- `cloudflare-deployer` (S5-Deployment) — ships the finished funnel package live once assets are approved
- `funnel-copy` (S9-Funnels) — hands off page-level copywriting once the asset map and funnel type are locked

### Fed By
- `funnel-planner` (S8-Meta) — supplies the initial funnel strategy and asset map before generation starts
- `offer-extraction` (S10-Copywriting) — supplies the offer details the orchestrator needs before it can confirm funnel type

### Feedback Loop
If a client reports a specific asset in the package underperforms (e.g. low opt-in rate), route that signal back to the individual generator skill for that asset (e.g. `landing-page-generator`), not back to the orchestrator itself — the orchestrator's job is sequencing and completeness, not per-asset optimization.

```yaml
chain_metadata:
  skill_slug: "funnel-builder-orchestrator"
