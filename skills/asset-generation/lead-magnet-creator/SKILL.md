...
license: proprietary
version: "1.0.0"
tags: ["saas", "funnel", "asset-generation", "lead-magnet", "opt-in", "content-creation"]
...
### Feeds Into
- `optin-funnel` (S9-Funnels) — the lead magnet is the core asset an opt-in funnel is built around
...
### Fed By
- `avatar-extraction` (S10-Copywriting) — supplies the target audience's pains and desires the lead magnet topic must match
- `offer-extraction` (S10-Copywriting) — supplies the core offer the lead magnet should logically lead into

### Feedback Loop
If opt-in rate stays low after the page and traffic are ruled out, the fix is usually the lead magnet's promised outcome, not its format — revisit the topic and headline before switching from PDF to checklist to video.

```yaml
chain_metadata:
  skill_slug: "lead-magnet-creator"
